...
export function searchSimilarFactsSameScope(db, embedding, scope, limit = 5, threshold = 0.85) {
    ...
    const totalActive = db.prepare('SELECT COUNT(*) AS n FROM facts WHERE is_active = 1 AND embedding_version = ?').get(EMBEDDING_VERSION).n;
    if (scopeCount === 0)
        return [];
    ...
    for (;;) {
        ...
        if (results.length >= limit || fetchCount >= totalActive)
            break;
