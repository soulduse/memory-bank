import { z } from 'zod';
import {
  CloudContextScopeType,
  CloudFactCategory,
  CloudSourceAgent,
  MemoryBankCloudHost,
  AsyncMemoryBankCloudHost,
...
export function listMemoryBankCloudMcpTools(options: ListMemoryBankCloudMcpToolsOptions = {}): MemoryBankCloudMcpTool[] {
  const adminTools: MemoryBankCloudMcpTool[] = [
    {
  ...
  const sessionTools: MemoryBankCloudMcpTool[] = [
    {
...
export async function callMemoryBankCloudMcpTool(host: MemoryBankCloudHost | AsyncMemoryBankCloudHost, name: string, args: unknown): Promise<string> {
  if (name === 'memory_bank_cloud_issue_token') {
    ...
    const issued = await host.issueLoginToken({
      ...
      account: {
        ...parsed.account,
        sourceAgent: parsed.account.sourceAgent as CloudSourceAgent,
      },
  ...
  if (name === 'memory_bank_cloud_put_context') {
    ...
    const entry = await host.putContext(parsed.sessionToken, {
      scopeType: parsed.scopeType as CloudContextScopeType,
  ...
  if (name === 'memory_bank_cloud_get_context') {
    ...
    const bundle = await host.getContextBundle(parsed.sessionToken, {
      query: parsed.query,
      limit: parsed.limit,
      includeScopes: parsed.includeScopes as CloudContextScopeType[] | undefined,
    });
  ...
  if (name === 'memory_bank_cloud_search') {
    ...
    const results = await host.searchExchanges(parsed.sessionToken, {
      ...
      projectPath: parsed.projectPath,
      sourceAgent: parsed.sourceAgent as CloudSourceAgent | undefined,
    });
  ...
  if (name === 'memory_bank_cloud_put_fact') {
    ...
    const fact = await host.putFact(parsed.sessionToken, {
      scopeType: parsed.scopeType as CloudContextScopeType,
      scopeId: parsed.scopeId,
      category: parsed.category as CloudFactCategory,
      fact: parsed.fact,
  ...
  if (name === 'memory_bank_cloud_search_facts') {
    ...
    const results = await host.searchFacts(parsed.sessionToken, {
      query: parsed.query,
      category: parsed.category as CloudFactCategory | undefined,
      limit: parsed.limit,
      includeScopes: parsed.includeScopes as CloudContextScopeType[] | undefined,
      projectId: parsed.projectId,
      sourceAgent: parsed.sourceAgent as CloudSourceAgent | undefined,
...
function accountJsonSchema(): Record<string, unknown> {
  return {
    type: 'object',
    properties: {
      tenantId: { type: 'string' },
...
function scopeJsonSchema(): Record<string, unknown> {
  return { type: 'string', enum: ['personal', 'project', 'team', 'org', 'company'] };
}

function factCategoryJsonSchema(): Record<string, unknown> {
  return { type: 'string', enum: ['decision', 'preference', 'pattern', 'knowledge', 'constraint'] };
}

function zodToJsonObject(properties: Record<string, unknown>, required: string[]): Record<string, unknown> {
  return {
    type: 'object',
    properties,
    required,
