...
export function isSlugProject(project: string | null | undefined): boolean {
 ...
 */
export function canonicalizeProject(db: Database.Database, project: string): string {
  ...
  try {
    ...
    `).all(project) as Array<{ cwd: string; n: number }>;
 ...
 */
export function autoHealScopeProjects(db: Database.Database): number {
  let healed = 0;
  try {
